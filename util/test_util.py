...
_criterionMSE = nn.MSELoss()
_L1Loss = nn.L1Loss()

#  PSNR = 10log10 (MAX^2/MSE)
#  10log10((2^b-1)^2/MSE)
def PSNR(input_a,input_b):
    mse = _criterionMSE(input_a, input_b)
    return -10 * log10(mse.item())
...
def _cat_and_unsqueeze(tensor_list:[torch.Tensor]):
    ret_tensor = None

    for index,each in enumerate(tensor_list):
        each = torch.unsqueeze(each,dim=0)
        if not index == 0:
            ret_tensor = torch.cat([ret_tensor,each],dim=0)
            continue
        ret_tensor = each
    return ret_tensor
...
def get_evaluate_indication(G,test_data_loader,device):
    with torch.no_grad():
        ...
        for each in result:
            _, b, c = torch.chunk(each, 3, dim=2)
            b_list.append(b)
            c_list.append(c)
        G.train()
